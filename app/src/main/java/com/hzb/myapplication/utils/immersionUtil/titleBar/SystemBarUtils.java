package com.hzb.myapplication.utils.immersionUtil.titleBar;
...
/**
 *  改工具类(immersionUtil 全屏 +图片背景+标题栏渐变+背景伸缩 )   可单独封装(不包括 titlebar)    lib 使用
 *
 *
 * 状态栏工具类
...
public class SystemBarUtils {
     ...
     * 创建假的透明栏
     */
    private static void setTranslucentViews(ViewGroup container,
                                           @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            View translucentView = container.findViewById(R.id.translucent_view);
            if (translucentView == null) {
